...
const promotionExit = runWrangler([
  "versions", "deploy", `${candidateVersionId}@100`,
  "--env", "production", "--yes", "--message", `GitHub Actions ${commit}`,
]);
...
annotate("warning", "Wrangler reported a promotion error; reconciling Cloudflare's active deployment state");
for (const delay of RECONCILE_DELAYS_SECONDS) {
  ...
  if (activeVersionId === candidateVersionId) {
    annotate("warning", "Cloudflare confirms the candidate was promoted; treating the release as successful");
    process.exit(0);
  }
...
annotate("error", "Promotion result is unknown after repeated Cloudflare status checks; inspect the deployment before retrying");
process.exit(promotionExit);
