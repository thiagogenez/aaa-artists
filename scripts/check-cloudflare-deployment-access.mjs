...
export async function checkCloudflareDeploymentAccess({ accountId, token }) {
  ...
  if (!Array.isArray(zones) || zones.length !== 1 || !zones[0]?.id) {
    throw new Error(
      `Expected one accessible ${ZONE_NAME} zone in the configured Cloudflare account`
    );
  }
  ...
  const domains = await cloudflareRequest(
    `/accounts/${encodeURIComponent(accountId)}/workers/domains?${domainQuery}`,
    token
  );
  ...
  if (JSON.stringify(actualHostnames) !== JSON.stringify(expectedHostnames)) {
    throw new Error(
      `Expected ${expectedHostnames.join(", ")} to target ${WORKER_NAME}; found ${actualHostnames.join(", ") || "none"}`
    );
  }
